...
import { chatAskQuestion } from './ask-question.ts';

const mockFetch = vi.fn();
global.fetch = mockFetch;

vi.mock('../../configs/backend.config.ts', () => ({
    getBackendUrl: vi.fn()
}));
  ...
  it('returns the answer when everything goes well', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        body: JSON.stringify({ question: 'My question ?', sessionId: 'session-123', transcribed: false })
      })
    );
  });
