...
import {chatAskQuestion} from "../queries/ask-question.ts";
import PreWrittenQuestions from "./PreWrittenQuestions.tsx";
...
import {getChatHistory} from "../queries/get-history.ts";
import {SyncLoader} from "react-spinners";
import {FaRotateLeft, FaTriangleExclamation} from "react-icons/fa6";
import {getChatBotSessionId, initNewChatBotSession} from "../services/chatBotSession.ts";
import {transcribeAudio} from "../queries/audio-transcribe.ts";
import {useAudioRecordingFix, formDataHasEmptyFile, rebuildFormDataWithBlob} from "../hooks/useAudioRecordingFix.ts";
...
const ChatBox = forwardRef<ChatBoxRef, ChatBoxProps>((props, ref) => {
  ...
  const refDeepChat = useRef<DeepChatCore | null>(null)
  ...
  const loadingBubbleRef = useRef({ toggle: (_config?: { role?: string }) => {} });
  const toggleLoading = (config?: { role?: string }) => loadingBubbleRef.current.toggle(config);
            ...
            ?
              <>
                ...
                <DeepChat
                  ref={refDeepChat}
                  history={history}
                  displayLoadingBubble={loadingBubbleRef.current}
                  microphone={{
                    ...
                    handler: async (body, signals) => {
                      ...
                      if (body instanceof FormData) {
                        ...
                        try {
                          ...
                          toggleLoading({ role: "ai" });

                          answer = await chatAskQuestion({messages: [newUserMessage]}, sessionId, true);
                        } catch (err) {
                            toggleLoading();
                            await signals.onResponse({text: t("errors.chatbot.transcriptionError"), role: "error"});
                            return;
                        }
